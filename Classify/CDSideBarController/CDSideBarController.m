...
- (CDSideBarController*)initWithImages:(NSArray*)images
{
    ...
    for (UIImage *image in [images copy])
    {
        ...
        button.frame = CGRectMake(10, 40 + (60 * index), 40, 40);
        button.tag = index;
...
- (void)insertMenuButtonOnView:(UIView *)view pointerSize:(CGRect)cgRect
{
    _menuButton.frame = CGRectMake(cgRect.origin.x, cgRect.origin.y, cgRect.size.width, cgRect.size.height);
    ...
    _backgroundMenuView.backgroundColor = [UIColor darkGrayColor]; //colorWithRed:1 green:1 blue:1 alpha:0.5f
...
- (void)performOpenAnimation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.4 animations:^{
            ...
            _menuButton.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -60, 0);
